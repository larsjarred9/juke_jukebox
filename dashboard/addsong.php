...
$stmt = $conn->prepare("SELECT artist, song, filename FROM stagedmusic WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($artist, $song, $file);
$stmt->fetch();
$stmt->close();
...
<body>
    ...
    <div class="container">
        <div class="card mt-4">
            <div class="card-body">
                <h2 class="text-white">Voeg nummer toe</h2>
                <hr>
                <form action="../php/middelwear/acceptsong.php?id=<?php echo $id; ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" name='categorie'>
                            <?php foreach ($categorie as $item) {
                                echo '<option value="' . $item["id"] . '">' . $item["categorie"] . '</option>';
                            } ?>
                        </select>
                    </div>
                    <br>
